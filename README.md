...
Documentation: [https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_security_for_kubernetes/4.10](https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_security_for_kubernetes/4.10)
...
| `purge-exceptions`     | 批量取消已审批或删除待审批的 false positive / deferral 例外           |
...
### 清理例外（purge-exceptions）

用于批量撤销本工具（或手工）创建的 **false positive** 与 **deferral** 例外，便于重跑 `apply` 前清空旧状态，或在测试环境回滚。

```bash
# 先 dry-run，仅列出将被处理的例外
DRY_RUN=true python3 -m acs purge-exceptions
# 或
DRY_RUN=true ./scripts/platform-fp-check.sh purge-exceptions

# 实际执行
python3 -m acs purge-exceptions
```
...
| 条件            | 值                                                |
| ------------- | ------------------------------------------------ |
| `targetState` | `FALSE_POSITIVE` 或 `DEFERRED`                    |
| `status`      | `PENDING`、`APPROVED` 或 `APPROVED_PENDING_UPDATE` |
...
| 状态                                    | API                                             | 说明          |
| ------------------------------------- | ----------------------------------------------- | ----------- |
| `APPROVED`                            | `POST /v2/vulnerability-exceptions/{id}/cancel` | 取消已生效例外     |
| `PENDING` / `APPROVED_PENDING_UPDATE` | `DELETE /v2/vulnerability-exceptions/{id}`      | 删除未生效或待更新请求 |


不会处理 `DENIED`、`CANCELLED`、`OBSERVED` 等状态，也不会触及 false positive / deferral 以外的例外类型。

输出审计日志：`data/results/exception-purge-<timestamp>.json`，含每条例外的 `id`、`target_state`、`status`、`action`（`cancel` / `delete`）、`image_scope`、`cves` 及执行结果（`ok` / `failed` / `dry_run`）。

> **注意**：`purge-exceptions` 为破坏性操作；生产环境请先 `DRY_RUN=true` 确认列表，再执行。该命令不在 `run` 流水线中自动调用。
...
- Deferral：`POST /v2/vulnerability-exceptions/deferral`（`exceptionExpiry.expiryType: TIME`，`expiresOn` 由 `DEFER_EXPIRY_MONTHS` 控制，默认当前 + 1 自然月）
...
- 按 image + **check reason** 聚合：同一 `(registry, remote, tag, reason)` 的多个 CVE 合并为一次请求；不同 `reason` 分开提交
- **imageScope.tag**：summary 中 digest 镜像的 `tag` 为 `sha256:...`（用于分组），提交 ACS API 时若 `tag` 以 `sha256` 开头则发送空字符串，否则原样发送
...
| `DEFER_EXPIRY_MONTHS`          | 否   | `1`                                                 | deferral 过期月数（`exceptionExpiry.expiresOn`）         |
...
- `VulnerabilityManagementRequests` **write**（创建 false positive / deferral 请求；`purge-exceptions` 删除 pending 请求）
- `VulnerabilityManagementApprovals` **write**（自动审批；`purge-exceptions` 取消已审批例外）
...
└── results/
    ...
    ├── exception-actions-<timestamp>.json  # FP/deferral 创建/审批审计日志
    └── exception-purge-<timestamp>.json    # purge-exceptions 清理审计日志
```
...
文档: [https://docs.redhat.com/en/documentation/red_hat_security_data_api/1.0/html-single/red_hat_security_data_api/index](https://docs.redhat.com/en/documentation/red_hat_security_data_api/1.0/html-single/red_hat_security_data_api/index)
